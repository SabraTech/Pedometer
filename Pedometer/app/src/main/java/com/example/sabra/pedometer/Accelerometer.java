...
public class Accelerometer extends AppCompatActivity implements SensorEventListener, View.OnClickListener {
    ...
    private double vibrateThreshold = 0.0;
    ...
    public void updateSteps(){
        ...
        double diffX = Collections.max(dataX) - Collections.min(dataX);
        double diffY = Collections.max(dataY) - Collections.min(dataY);
        double diffZ = Collections.max(dataZ) - Collections.min(dataZ);
        ...
        if(workingAxis == 0){
            double average = (Collections.max(dataX) + Collections.min(dataX)) / 2;
            ...
            double average = (Collections.max(dataY) + Collections.min(dataY)) / 2;
            for(int i = 0, j = 1; i < 49; i++, j++){
            ...
            double average = (Collections.max(dataZ) + Collections.min(dataZ)) / 2;
            for(int i = 0, j = 1; i < 49; i++, j++){
